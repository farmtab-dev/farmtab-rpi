...
function saveImageLocallyOnlyUsingCV(img_filename, cam_serial, image) {
    const output_filepath = CONFIG.dirpath.img + cam_serial + " " + img_filename + ".jpg";
    // https://www.npmjs.com/package/opencv4nodejs?activeTab=readme
    try {
        cv.imwrite(output_filepath, image);

        console.log("Success SAVE [ " + img_filename + " ] image LOCALLY using CV. ");
    } catch (e) {
        log.error("\nError save Image at temporary folder");
        console.log(e)
    }
}