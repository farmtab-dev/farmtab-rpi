...
const {
    ...
    ExposureMode,
    AwbMode
} = require("pi-camera-connect"); // https://github.com/launchcodedev/pi-camera-connect
const fs = require('fs');
...
log.warn("Connecting to ", CONFIG.udp_socket_host, ":", CONFIG.udp_socket_port, " as ", camDesc);
const udp_client = udp.createSocket('udp4') // creating a client socket
...
udp_client.on('message', (msg, info) => {
    console.log('Data received from server : ' + msg.toString())
    console.log('Received %d bytes from %s:%d\n', msg.length, info.address, info.port)
    switch (msg.toString()) {
        case "client info req":
            publishUDPData({
                topic: "client info res",
                data: {
                    client_name: CAM_SERIAL,
                    cam_serial: CAM_SERIAL,
                    client_type: "camera",
...
publishUDPData({
    topic: "reply-snapshotclient info res",
    data: {
        sample: "HELLO"
    }
})

function publishUDPData(data) {
    udp_client.send(data, CONFIG.udp_socket_port, CONFIG.udp_socket_host, error => {
        if (error) {
            console.log("UDP ERROR: ", error);
            publishUDPData(data);
        } else {
            switch (data.topic) {
                case "client info res":
                    console.log("Send camera client info - ", camDesc);
                    break;
                case "incoming image":
                    console.log("Sent camera feed for processing");
                    break;
                case "publish-camera-state":
